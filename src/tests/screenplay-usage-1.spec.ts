...
import { AdminAppHomeMenuIsVisible, WebAppHomeMenuIsVisible } from '../screenplay/user-flows/login/login.question';
import {
    CreateUserApiTask,
...
test.describe('Feature: Login as Admin and New User', () => {
    test.beforeEach(async ({ admin }) => {
        admin.useOrg(Org.COACHING_QA_LEGACY);
    });
    ...
    test('Scenario: Admin user should be able to login', async ({ admin }) => {
        ...
        await test.step('Then Admin verifies if home button is visible', async () => {
            const isHomeButtonVisible = await admin.asks(AdminAppHomeMenuIsVisible.answer());
    ...
    test('Scenario: New user should be able to login', async ({ admin }) => {
        ...
        await test.step('Then New user verifies if home button is visible', async () => {
            const isHomeButtonVisible = await userActor.asks(WebAppHomeMenuIsVisible.answer());
